...
#include <ttkimplicitQuadrangulation.h>
...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("implicitQuadrangulation");

  // ---------------------------------------------------------------------------
  // Initialize ttkimplicitQuadrangulation module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto implicitQuadrangulation = vtkSmartPointer<ttkimplicitQuadrangulation>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // implicitQuadrangulation->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkimplicitQuadrangulation filter
      implicitQuadrangulation->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    implicitQuadrangulation->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkimplicitQuadrangulation filter
  // ---------------------------------------------------------------------------
  implicitQuadrangulation->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < implicitQuadrangulation->GetNumberOfOutputPorts(); i++) {
      auto output = implicitQuadrangulation->GetOutputDataObject(i);
